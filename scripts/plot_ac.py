...
def get_soma_adjacent_segments(adj_matrix, seg_data, soma_index=0):
    """
    Create a dictionary of segment types based on the segments that are adjacent to the soma
    and includes the soma itself.

    Parameters:
    - adj_matrix: A numpy array representing the adjacency matrix (nseg x nseg) where a 1 indicates
                  that the row segment is parent to the column segment.
    - soma_index: The index of the soma segment in the adjacency matrix.
    - seg_data: A dictionary containing segment data, where 'section' is a key that maps to a list or array of segment types.

    Returns:
    A dictionary with keys being segment type names (inferred from segment names) and values being lists of segment indices.
    """
    import numpy as np
    
    segment_types = {"Soma": [soma_index]}  # Initialize with the soma
    
    # Get the indices of segments adjacent to the soma
    adjacent_indices = np.where(adj_matrix[soma_index, :] == 1)[0]
    
    for adj_seg_index in adjacent_indices:
        seg_type = seg_data['section'][adj_seg_index]

        # Prefix the type with "Soma_Adj_"
        soma_adj_type = "Soma_Adj_" + seg_type.capitalize()
        ...
        segment_types[seg_type].append(adj_seg_index)
        segment_types[soma_adj_type].append(adj_seg_index)
...
def get_segments_of_type(seg_data, segment_type):
	return [seg for seg in segments if segment_type in seg_data['section'][seg]]
...
def analyze_currents(sim_directory):
    ...
    t = np.arange(0, len(v[0]) * parameters.h_dt, parameters.h_dt)
    ...
    analysis.compute_axial_currents(v, seg_data, adj_matrix)
  
	# Finding segments
    soma_adj_segs = get_soma_adjacent_segments(adj_matrix, seg_data, soma_index=0)
