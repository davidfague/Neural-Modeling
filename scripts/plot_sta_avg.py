...
def _analyze_spike_relationships(sim_directory, spike_type, wrt_spike_type, section, elec_dist, multi_sim=False):
    v = analysis.DataReader.read_data(sim_directory, "v")
    soma_spikes = analysis.DataReader.read_data(sim_directory, "soma_spikes")
    ica = analysis.DataReader.read_data(sim_directory, "ica")
    inmda = analysis.DataReader.read_data(sim_directory, "i_NMDA")
    ...
    try:
        ...
        if multi_sim:
            binned_sta, quantiles = _map_stas_to_quantiles_and_plot(
                sta=sta,
                spikes=spikes,
                section=section,
                elec_dist_from=elec_dist,
...
def analyze_all_spike_relationships(sim_directory):
    ...
    if type(sim_directory) == list: # update # multi_sim
            ...
            for wrt_spike_type in spike_types:
                for section in sections:
                    for elec_dist in elec_dists:
                        ...
                        stas=[]
                        all_quantiles=[]
                        for sim_path in sim_directory:
                            print(f"sim_path:{sim_path}")
                            soma_spikes = analysis.DataReader.read_data(sim_path, "soma_spikes")
                            parameters = analysis.DataReader.load_parameters(sim_path)
                            binned_sta, quantiles = _analyze_spike_relationships(sim_path, spike_type, wrt_spike_type, section, elec_dist, multi_sim = True)
                            stas.append(sta)
                            all_quantiles.append(quantiles)
                            
                        sta = np.mean(stas)
                        new_quantiles = np.mean(quantiles)
                        title=f"{section}-{spike_type}"
                        fig = _plot_sta(sta_binned, quantiles, title=title, wrt_spikes_type=wrt_spike_type, elec_dist=elec_dist)
                        plt.show()
                        if save:
                            fig.savefig(os.path.join(sim_directory, f"{elec_dist_from}_{title}_wrt{wrt_spike_type}.png"), dpi = fig.dpi)
...
if __name__ == "__main__":
    ...
    if single_sim:
        ...
        sim_paths = []
        for sim_dir in os.listdir(sim_directory):
            sim_path = os.path.join(sim_directory, sim_dir)
            sim_paths.append(sim_path)
        analyze_all_spike_relationships(sim_paths)
