...
def _compute_sta_for_each_train_in_a_list(list_of_trains, spikes, win_length=60, parameters=None):
    stas = []
    for train in list_of_trains:
        if len(train) == 0:
            stas.append(np.zeros((1, win_length), dtype=np.float32))
            continue
        cont_train = np.zeros(int(parameters.h_tstop / parameters.h_dt), dtype=np.float32)
        cont_train[train] = 1
        ...
        sta = (sta - np.mean(cont_train)) / (np.mean(cont_train) + 1e-15) * 100
...
def _map_stas_to_quantiles_and_plot(
    sta, spikes, section, elec_dist_from, title, xlabel_spike_type, cbar_spike_type,
    sim_directory, save, save_directory, indexes=None):
...
def _analyze_spike_relationships(sim_directory, spike_type, wrt_spike_type, section, elec_dist, parameters, save, save_directory):
    # Load all arrays up front, downcast to float32
    ...
    try:
        ...
        sta = _compute_sta_for_each_train_in_a_list(spikes, wrt_spikes, parameters=parameters)
            ...
            for section in sections:
                for elec_dist in elec_dists:
                    ...
                    _analyze_spike_relationships(
                        sim_directory, spike_type, wrt_spike_type, section, elec_dist,
                        parameters, save, save_directory)
