...
if __name__ == "__main__":
    ...
    for amp in np.arange(-2, 2.1, 0.2):  # 2.1 ensures that 2.0 is included in the range
        ...
        sim.submit_job(HayParameters(f"Detailed_FI_amp{rounded_amp}", h_tstop= 2000, reduce_cell = True, expand_cable = True, all_synapses_off=True, record_ecp=False,
        CI_on=True, h_i_amplitude=rounded_amp, h_i_duration=1950, h_i_delay=50))
        sim.submit_job(HayParameters(f"Reduced_FI_amp{rounded_amp}", h_tstop= 2000, reduce_cell = True, expand_cable = True, all_synapses_off=True, record_ecp=False,
        CI_on=True, h_i_amplitude=rounded_amp, h_i_duration=1950, h_i_delay=50))
