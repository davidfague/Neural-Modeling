...
def main(numpy_random_state, neuron_random_state, cluster_index_to_stim):
...
if __name__ == "__main__":
    ...
    for np_state in constants.numpy_random_states:
        for neuron_state in constants.neuron_random_states:
            for cluster_index_to_stim in range(0,constants.number_of_presynaptic_cells+1):
                    if constants.parallelize:
                        pool.append(Process(target = main, args=[np_state, neuron_state, cluster_index_to_stim]))
                    else:
                        p = Process(target = main, args=[np_state, neuron_state, cluster_index_to_stim])
                        p.start()
                        p.join()
                        p.terminate()
