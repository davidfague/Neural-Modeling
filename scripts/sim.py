...
if __name__ == "__main__":
    ...
    if in_vitro:
      #sim1 = Simulation(SkeletonCell.Hay) # NR
      #sim2 = Simulation(SkeletonCell.Hay) # DR
      for amp in [-2.0,-1.0,-0.5,0.0,0.25,0.5,0.6,0.75,1.0,1.5,2.0]:
        ...
        #sim1.submit_job(HayParameters(sim_name=f"FI_HAY_NR_{int(amp*1000)}", reduce_cell=True, expand_cable=False, 
        #  CI_on=True, h_i_amplitude=amp, h_i_duration=10000, h_i_delay=1000, h_tstop=11000))
        #sim2.submit_job(HayParameters(sim_name=f"FI_HAY_DET_{int(amp*1000)}", reduce_cell=False, expand_cable=False, 
        #  CI_on=True, h_i_amplitude=amp, h_i_duration=10000, h_i_delay=1000, h_tstop=11000))
