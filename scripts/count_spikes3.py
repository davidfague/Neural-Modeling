...
def _analyze_Na(ben):
    ...
    # Use log spacing for binning but keep the bins uniform in the plot
    duration_bins = np.logspace(np.log10(0.15), np.log10(1.05), num=15)
...
def _analyze_Ca(ben):
    ...
    # Use log spacing for binning but keep the bins uniform in the plot
    duration_bins = np.logspace(np.log10(2*10**1.1), np.log10(2*10**1.5), num=15)
...
def _analyze_NMDA(ben):
    ...
    # Use log spacing for binning but keep the bins uniform in the plot
    duration_bins = np.logspace(np.log10(2*10**1.1), np.log10(2*10**1.8), num=15)
...
def _analyze_all_and_plot(ben=False):
    ...
    for i, axname in enumerate(['left', 'center', 'right']):
        im = ax[axname].pcolormesh(np.arange(len(results[i][2])), np.arange(len(results[i][1])), results[i][0])
        ...
        if i == 0:
            ...
            # Set logarithmic ticks manually
            log_ticks = np.logspace(np.log10(0.15), np.log10(1.05), num=5)
            ax[axname].set_xticks(np.linspace(0, len(results[i][2])-1, num=5))
            ax[axname].set_xticklabels([f"{x:.2f}" for x in log_ticks])
            ax[axname].set_yticks(np.linspace(0, len(results[i][1])-1, num=5))
            ax[axname].set_yticklabels([f"{x:.4f}" for x in np.logspace(-3, -2, num=5)])
        ...
        if i == 1:
            ...
            # Set logarithmic ticks manually
            log_ticks = np.logspace(np.log10(2*10**1.1), np.log10(2*10**1.8), num=5)
            ax[axname].set_xticks(np.linspace(0, len(results[i][2])-1, num=5))
            ax[axname].set_xticklabels([f"{int(x):d}" for x in log_ticks])
            ax[axname].set_yticks(np.linspace(0, len(results[i][1])-1, num=5))
            ax[axname].set_yticklabels([f"{x:.2f}" for x in np.logspace(-1, 0.5, num=5)])
        ...
        if i == 2:
            ...
            # Set logarithmic ticks manually
            log_ticks = np.logspace(np.log10(2*10**1.1), np.log10(2*10**1.5), num=5)
            ax[axname].set_xticks(np.linspace(0, len(results[i][2])-1, num=5))
            ax[axname].set_xticklabels([f"{int(x):d}" for x in log_ticks])
            ax[axname].set_yticks(np.linspace(0, len(results[i][1])-1, num=5))
            ax[axname].set_yticklabels([f"{x:.2f}" for x in np.linspace(0.1, 1.4, num=5)])
