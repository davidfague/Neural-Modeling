...
# already_used_seeds = [1000, 100000, 500000, 10000000]
# todo_seeds = [5000, 10000, 50000, 5000000, 1000000, 4444, 4444444, 9999]
numpy_random_states = [99999] #999999, 9999999, 999999999, 4444, 4444444]  # Add more seeds if needed (can do multiple)
neuron_random_states = None
# now simulation is defined in slurm_sim.py so this is not used: sim_title = 'BenSynapses_testing_depth_of_mod_more_seeds'#'BenSynapses_final_detailed_syn_dist_analysis'
...
all_depth_of_mod_range = [0, 0.05, 0.1, 0.25, 0.5, 0.75, 1]
...
if __name__ == "__main__":
    ...
    # Self-check
    assert len(all_parameters) == N_workers, f"Mismatch in lengths: len(all_parameters) is {len(all_parameters)}, but N_workers is {N_workers}."

    # Generate parameter pickles
    if os.path.exists("params"):
        shutil.rmtree("params")
    os.mkdir("params")
    for pid in range(len(all_parameters)):
        with open(os.path.join("params", f"{pid}.pickle"), 'wb') as file:
            pickle.dump(all_parameters[pid], file)
